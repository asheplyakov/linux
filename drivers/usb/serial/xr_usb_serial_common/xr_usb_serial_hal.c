...
int xr_usb_serial_set_line(struct xr_usb_serial *xr_usb_serial, struct usb_cdc_line_coding* line)
 	                                     ...
 	                                     struct tty_struct *tty, unsigned int cflag)
 {
	...
	if(xr_usb_serial->rs485_mode)
	{
		gpio_mode = UART_GPIO_MODE_SEL_RS485;
		if(xr_usb_serial->rs485_den_active_high)
			gpio_mode |= UART_GPIO_MODE_RS485_POL;
			
	}else if (cflag & CRTSCTS)
	{
...
int xr_usb_serial_fifo_reset(struct xr_usb_serial *xr_usb_serial)
{
		...
		if((xr_usb_serial->DeviceProduct & 0xfff0) == 0x1400)
		{
			ret = xr_usb_serial_set_reg(xr_usb_serial, xr_usb_serial->reg_map.uart_rx_fifo_reset, 0x1);
			ret |= xr_usb_serial_set_reg(xr_usb_serial, xr_usb_serial->reg_map.uart_tx_fifo_reset, 0x1);
		}
...
static void init_xr21b140x_reg_map(void)
{
	...
	xr21b140x_reg_map.uart_rx_fifo_reset = 0x23;
	xr21b140x_reg_map.uart_tx_fifo_reset = 0x20;
}
