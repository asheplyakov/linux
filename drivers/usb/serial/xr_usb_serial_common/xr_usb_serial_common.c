...
static ssize_t show_rs485_mode
(struct device *dev, struct device_attribute *attr, char *buf)
{
	int rv = -EINVAL;
	uint16_t data;
	...
	return sprintf(buf, "%d\n", ((data & 0x7) == UART_GPIO_MODE_SEL_RS485) ? 1 : 0);
}

static ssize_t store_rs485_mode
(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	{
		data &= 0x7;
		data |= UART_GPIO_MODE_SEL_RS485;
	} else if(buf[0] == '0') {
		data &= 0x7;
		data |= UART_GPIO_MODE_SEL_GPIO;
	} else return -EINVAL;

	rv = xr_usb_serial_set_reg(xr_usb_serial, xr_usb_serial->reg_map.uart_gpio_mode_addr, data);
	if(rv < 0)
		return rv;
...
static DEVICE_ATTR(rs485_mode, S_IWUSR | S_IRUGO, show_rs485_mode, store_rs485_mode);

static ssize_t show_rs485_den_active_high
(struct device *dev, struct device_attribute *attr, char *buf)
{
	int rv = -EINVAL;
	uint16_t data;
	...
	return sprintf(buf, "%d\n", ((data & UART_GPIO_MODE_RS485_POL) ? 1 : 0));
}

static ssize_t store_rs485_den_active_high
(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	uint16_t data;
	int rv;
...
static DEVICE_ATTR(rs485_den_active_high, S_IWUSR | S_IRUGO, show_rs485_den_active_high, store_rs485_den_active_high);
...
made_compressed_probe:
	...
	i = device_create_file(&intf->dev, &dev_attr_rs485_mode);
	if (i < 0)
		goto alloc_fail8;

	i = device_create_file(&intf->dev, &dev_attr_rs485_den_active_high);
	if (i < 0)
		goto alloc_fail9;
...
skip_countries:
	...
	return 0;
alloc_fail9:
	device_remove_file(&xr_usb_serial->control->dev, &dev_attr_rs485_mode);
alloc_fail8:
	if (xr_usb_serial->country_codes) {
...
static void xr_usb_serial_disconnect(struct usb_interface *intf)
{
	...
	device_remove_file(&xr_usb_serial->control->dev, &dev_attr_rs485_den_active_high);
	device_remove_file(&xr_usb_serial->control->dev, &dev_attr_rs485_mode);
	device_remove_file(&xr_usb_serial->control->dev, &dev_attr_bmCapabilities);
