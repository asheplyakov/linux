...
int smi_connector_get_modes(struct drm_connector *connector)
{
	...
	if(g_specId == SPC_SM750)
	{
		...
		if(connector->connector_type == DRM_MODE_CONNECTOR_VGA)
		{
			...
			tmpedid = &edid1;
			ddk750_edidReadMonitorEx(SMI1_CTRL, tmpedid, 128, 0, 17, 18);
...
//          hw768_get_hdmi_edid(tmpedid);   // Too Slow..
			...
			drm_edid_to_eld(connector, &hdmi_edid);
		}
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(4,4,0)	 //use detect monitor function to do detection. But it seem only work under Ubuntu16.04.1
...
int smi_modeset_init(struct smi_device *cdev)
{
	...
	if(smi_bpp >= 24 || g_specId == SPC_SM768)
		smi_bpp = 32;
