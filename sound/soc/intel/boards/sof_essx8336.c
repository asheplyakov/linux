...
#if 0
#define KDP {printk("in %s %s() @ %d",__FILE__,__func__,__LINE__);}
#else
#define KDP {}
#endif
...
//	struct snd_soc_component *codec = codec_dai->component;
	...
	snd_soc_dai_set_tdm_slot(codec_dai, 0x3, 0x3, 2, 24);
...
static int sof_es8336_trigger(struct snd_pcm_substream *substream, int cmd)
{
	printk("%s %s",substream->name,substream->pcm->name);
	return 0;
}
...
static int sof_es8336_probe(struct platform_device *pdev)
{
	...
	printk("dailink[%d].codecs->name == %s\n",
		dai_index, dailink[dai_index].codecs->name);
	printk("dailink[%d].codecs->dai_name == %s\n",
		dai_index, dailink[dai_index].codecs->dai_name);
	printk("dailink[%d].name == %s\n",
		dai_index, dailink[dai_index].name);
	printk("dailink[%d].stream_name == %s\n",
		dai_index, dailink[0].stream_name);
	printk("platform number: %d ; name: %s ; dainame: %s",
		dailink[dai_index].num_platforms,
		dailink[dai_index].platforms->name,
		dailink[dai_index].platforms->dai_name);
